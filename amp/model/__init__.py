...
class LossFunction:
    ...
    def process_parallels(self, vector, server, n_pids, keys, args):
        ...
        print('finished')
        print(finished)
        print('results before sending sent with send_pyobj')
        print(results)
        while not finished.all():
            message = server.recv_pyobj()
            if message['subject'] == '<purpose>':
                print("server.send_string('calculate_loss_function')")
                print('')
                server.send_string('calculate_loss_function')
            elif message['subject'] == '<request>':
                print("elif message['subject'] == '<request>':")
                print("request = message['data']  # Variable name.")
                request = message['data']  # Variable name.
                print(request)
                print('')
                if request == 'images':
                    print("if request == 'images':")
                ...
                print("elif message['subject'] == '<result>':")
                result = message['data']
                print("result = message['data'] received received with send_pyobj")
                print(result)
                print('')
            ...
            print('final results returned in this within the class')
            print(results)
            print('')
            print('')
        return results
    ...
    def check_convergence(self, loss, energy_loss, force_loss,
        ...
        if p.force_coefficient != 0.:
            ...
            if p.convergence['force_rmse'] is not None:
                ...
                print("if p.convergence['force_rmse'] is not None:")
                print(force_rmse)
            ...
            if self.log_losses:
                ...
                print("if self.log_losses:")
                print("loss, energy_rmse, energy_maxresid, force_rmse, force_maxresid")
                print(loss, energy_rmse, energy_maxresid, force_rmse, force_maxresid)

            self._step += 1
            print("This returns the following")
            print(energy_rmse_converged, energy_maxresid_converged, force_rmse_converged, force_maxresid_converged)
            ...
            self._step += 1
            print("last else:")
            print(energy_rmse_converged, energy_maxresid_converged)
