...
#    Stanford University
#    zulissi@gmail.com
# Help/testing/discussions: Andrew Doyle (Stanford) and the AMP development team

# This module implements energy- and force- training using Google's TensorFlow library. In doing so, the training is multithreaded and GPU accelerated.
...
class tfAmpNN:
    def __init__(self, elementFingerprintLengths, hiddenlayers=[5, 5], activation='relu', keep_prob=0.5, RMSEtarget=1e-2, maxTrainingEpochs=10000,batchsize=20,initialTrainingRate=1e-4,miniBatch=True,tfVars=None,saveVariableName=None,parameters={},sess=None,maxAtomsForces=0):
        #Inputs:
        # elementFingerprintLengths: dictionary (one for each element type) that contains the fingerprint length for each element
        # hiddenlayers: structure of the neural network
        # activation: activation type
        # keep_prob: dropout rate for the neural network to reduce overfitting.  (keep_prob=1. uses all nodes, keep_prob~0.5-0.8 better for training)
        # RMSEtarget: target for the final loss function
        # maxTrainingEpochs: maximum number of times to loop through the training data before giving up
        ...
        # saveVariableName: name used for the internal tensorflow variable naming scheme.  if variables have the same name as another model in the same tensorflow session, there will be collisions
        # sess: tensorflow session to use (None means start a new session)
        # maxAtomsForces: number of atoms to be used in the force training.  It sets the upper bound on the number of atoms that can be used to calculate the force for (e.g. if maxAtomsForces=40, then forces can only be calculated for images with less than 40 atoms)
    ...
    # constructModel goes through and sets up the tensorflow neural networks for each atom type
    def constructModel(self):
        ...
        self.loss_forces=self.forcecoefficient*tf.sqrt(tf.reduce_mean(tf.square(tf.sub(self.forces_in, self.forces))))

        #Define the training step for force training
        self.totalloss=self.loss_forces+self.loss
        self.train_step_forces = tf.train.AdamOptimizer(self.learningrate,beta1=0.9).minimize(self.totalloss)

    # this function resets all of the variables in the current tensorflow model
    def initializeVariables(self):
        self.sess.run(tf.initialize_all_variables())

    # This function generates the input dictionary that maps various inputs on the python side to placeholders for the tensorflow model
	...
	keylist=images.keys()
        ...
        #simple model to help normalize the energies by guessing a per-atom energy.  This is helpful to removing the large electronic energy associate for each element, making it easier to regress later
        model_ransac = sklearn.linear_model.RANSACRegressor(sklearn.linear_model.LinearRegression(),residual_threshold=outlier_energy,min_samples=0.1)
	...
	atomArraysAll, nAtomsDict, atomsIndsReverse, natoms,atomArraysAllDerivs = generateTensorFlowArrays(
        ...
        if self.maxAtomsForces==0:
            if force_coefficient is not None:
                forces=map(lambda x: images[x].get_forces(apply_constraint=False),keylist)
            else:
                forces=0.
        ...
        def trainmodel(targetRMSE, trainingrate, keepprob, maxepochs):
            ...
            #continue taking training steps as long as we haven't hit the RMSE minimum of the max number of epochs
            while (RMSE_total > targetRMSE) & (icount < maxepochs):
                ...
                if icount_global% 10==0:
                    feedin=self.generateFeedInput(range(len(keylist)), energies, atomArraysAll,atomArraysAllDerivs,
                                                  nAtomsDict, atomsIndsReverse, len(keylist), trainingrate, keepprob, natoms,forcesExp=forces,energycoefficient=energy_coefficient,forcecoefficient=force_coefficient)
                    RMSE = self.loss.eval(feed_dict=feedin) * self.parameters['energyProdScale']
                    log('global RMSE=%1.3f'%(RMSE))
                    if force_coefficient>1.e-5:
                        RMSE_total=self.totalloss.eval(feed_dict=feedin)* self.parameters['energyProdScale']
                        log('combined loss function (energy+force)=%1.3f'%(RMSE_total))
                    else:
                        RMSE_total=RMSE
    ...
    def get_energy_list(self, hashs, fingerprintDB,fingerprintDerDB=None,keep_prob=1.,forces=False):
        ...
        for element in self.elements:
            if len(atomArraysFinal[element]) > 0:
                ...
                feedinput[self.tensorDerivDict[element]] = np.zeros((1,1,3,self.elementFingerprintLengths[element]))
...
#The essential problem is that each neural network is associated with a specific element type.  Thus, atoms in each ASE image need to be sent to different
